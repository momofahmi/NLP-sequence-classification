...
domain-specific notebooks is *copied in* into main.ipynb so the marker
can read the entire pipeline in one place. The notebook also defaults to
*loading the results* from JSON files we already produced (Joel's
...
BESSTIE: sentiment & sarcasm classification across three English varieties (UK, AU, IN).

Group PG15 - Sumtally Umme Yusrah, Mohamed Fahmi Ahmed, Joel Allen-Caliste,
Sayed Omar Aabid, Mohammad Hossein Modaresi, Fiyinfoluwa Akano.

Dataset: `surrey-nlp/BESSTIE-CW-26` (Srirag et al., 2025).

We pull together the work from each of our individual notebooks
(`1.1_EDA*`, `2.1_Baseline*`, `2.2_RoBERTa*`, `2.3_LoRA*`) into one place.
By default this loads the saved training results from `reports/results/` and
just regenerates the plots, which takes about a minute. To retrain from
scratch, flip `FROM_SCRATCH = True` near the top (~1 hour on a Colab T4).
'''))
...
cells.append(code(r'''# Config flags
SEEDS = [42, 123]
...
Variety/source/split distributions, sarcasm vs sentiment correlation, POS comparison and slang markers. All from `src/eda_distributions.py`.'''))
...
Pairwise vocabulary overlap (Jaccard) and TF-IDF cosine across the three varieties.'''))
...
TF-IDF (15k features, unigrams + bigrams) with class-balanced LR and LinearSVC, one classifier per task. LR and SVM end up within 0.01 Macro-F1 of each other so we just use LR going forward.'''))
...
cells.append(code(r'''# Per-variety LR breakdown (Table 8 in the report)
per_var_rows = []
...
# -----------------------------------------------------------------------------
# Section 2.2 - RoBERTa cross-variety
# -----------------------------------------------------------------------------
cells.append(md(r'''### 2.2 RoBERTa cross-variety

Five training conditions: `uk_only`, `au_only`, `in_only`, `inner_pool` (UK+AU), and `all`. Each one is evaluated on every variety's test set (5x3 matrix, 2 seeds, 30 runs total).

Functions below come from `notebooks/2.2_RoBERTa_CrossVariety_Joel_Fiyin.ipynb`.'''))
...
cells.append(md(r'''**Cross-variety experiment loop - runs 5 conditions x 3 test sets x 2 seeds = 30 evaluations**

This cell is *only* executed when `FROM_SCRATCH=True`. By default we skip it and load the cached `weighted_results/*.json` files (committed to the repo) in the next cell - those are the numbers used in the report.'''))
...
cells.append(code(r'''# Confusion matrix for the best condition (Figure 33)
best_condition = max(
    ROBERTA_CONDITIONS,
...
cells.append(code(r'''# Per-class F1 across all 5 conditions (Figure 32)
records = []
for cond in ROBERTA_CONDITIONS:
...
Same `train_roberta()` as above with `label_col="Sentiment"`. Saved to `reports/results/roberta_sentiment/all_pool.json`.'''))
...
# Section 2.3 - LoRA
# -----------------------------------------------------------------------------
cells.append(md(r'''### 2.3 LoRA adapters - OPT-1.3B (main model)

Three per-variety adapters on `facebook/opt-1.3b`, `r=8, alpha=16, lr=2e-4`, weighted CE. About 1.6 M trainable parameters out of 1.32 B base (~0.12%). Helpers come from `models/lora/lora_adapters.py`.'''))
...
cells.append(md(r'''Training and evaluation helpers for the per-variety adapters.'''))
...
cells.append(code(r'''# only run if FROM_SCRATCH=True (~30 min on Colab T4)
if FROM_SCRATCH:
...
cells.append(md(r'''**Load saved LoRA JSON and rebuild the cross-variety heatmap (Tables 6-7 / Figure 34).**

JSON shape::
```json
{
  "base_model": "Qwen/Qwen2.5-1.5B",        // or facebook/opt-1.3b for the run
  "macro_f1": {
...
Cross-task and per-variety summary from the JSONs loaded above. No new training here.'''))
...
Best adapter is OPT-1.3B + en-AU (highest Sarcastic-F1 in Section 3.4). Steps:
...
`reports/results/q4_errors.json` is committed to the repo with the 10
specific examples discussed in the report (4 explained, 6 held-out).
With `RUN_ERROR_ANALYSIS=True`, the cell below regenerates the same file
from the dataset using `scripts/q4_build_curated_errors.py`. Switch to
`scripts/q4_extract_errors.py` (commented out) to re-run the actual model
and pull a fresh batch.'''))
...
cells.append(code(r'''# # load the curated errors file. If it exists but has 0 explanations
...
`app/app.py` keeps a single frozen OPT-1.3B in memory and swaps the LoRA adapter when the user picks a different variety. To run locally:
...
Screenshots used in the report are in `reports/figures/q5_1_deployment_*.png`.'''))
...
cells.append(code(r'''# # benchmark wants three model flags - if we don't pass them every model
...
If we got here, the full pipeline ran and every figure/table the report references has been regenerated.

Reproducibility notes:
- Two random seeds (42, 123) used in every training cell, std reported across seeds.
- All hyperparameters live at the top of the notebook (`Section 0.1 Config flags`) or alongside the relevant function definition.
- All datasets are loaded from the official `surrey-nlp/BESSTIE-CW-26` Hugging Face mirror.
- All adapters are pushed to `huggingface.co/momofahmi/*`, and the run reads the cached JSONs in `reports/results/`.
- For full retraining, set `FROM_SCRATCH=True` in Section 0.1 and run again on a Colab T4.

Where each section lives in the report:
- Section 1 <-> Section 1.1 / Section 1.2 above
- Section 2 <-> Section 2.1 / Section 2.2 / Section 2.3 above
- Section 3 <-> Section 3 above
- Section 4 <-> Section 4 above
- Section 5 <-> Section 5 above
'''))
